...
#  Required:
#
#  --chim_summary_file <string>     chimera summary file
#
...
#
#  --exclude_paralogs               exclude gene paralogs from consideration
#
###############################################################################################
...
&GetOptions ( 'h' => \$help_flag,
              ...
              'chim_summary_file=s' => \$chim_summary_file,
...
unless ($chim_summary_file && $gtf_file && $genome_fasta_file) {
    die $usage;
}

main: {
        
    my @chim_pairs = &get_chim_gene_pairs($chim_summary_file, $exclude_paralogs_flag);
    
    my %in_fusion;
    foreach my $chim_pair (@chim_pairs) {
        my ($geneA, $geneB) = @$chim_pair;
        $in_fusion{$geneA}++;
        $in_fusion{$geneB}++;
    }
...
####
sub get_chim_gene_pairs {
    my ($chim_summary_file, $exclude_paralogs_flag) = @_;

    my %fusion_pairs;

    open (my $fh, $chim_summary_file) or die "Error, cannot open file $chim_summary_file";
    while (<$fh>) {
        if ($exclude_paralogs_flag && /PARALOGS/) {
            next; 
        }
        
        chomp;
        my @x = split(/\t/);
        my ($geneA, $geneB) = ($x[2], $x[5]);
        my $key = join("$;", $geneA, $geneB);
        $fusion_pairs{$key}++;
    }
    close $fh;

    my @fusion_pairs_list;
    foreach my $key (keys %fusion_pairs) {
        my ($geneA, $geneB) = split(/$;/, $key);
        push (@fusion_pairs_list, [$geneA, $geneB]);
    }

    return(@fusion_pairs_list);
}
