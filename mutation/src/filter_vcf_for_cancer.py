...
# Arguments
prsr_arguments = argparse.ArgumentParser( prog = "filter_vcf_for_cancer.py", description = "Extracts common variants which do not have COSMIC ids entries from a vcf file", formatter_class = argparse.ArgumentDefaultsHelpFormatter )
  ...
  with open( args.str_output_file, "w" ) as hndl_out:
    for lstr_line in csv.reader( hndl_vcf, delimiter = STR_VCF_DELIMITER ):
      ...
      if STR_COMMON_VARIANT in dict_info_tokens:
        if ( dict_info_tokens[ STR_COMMON_VARIANT ] == "1" ) and ( STR_COSMIC_ID not in dict_info_tokens ):
          i_common = i_common + 1
          continue

      ## Filter DP < 10
      if STR_DEPTH in dict_info_tokens:
        if ( int( dict_info_tokens[ STR_DEPTH ] ) < 10 ) and ( STR_COSMIC_ID not in dict_info_tokens ):
          i_depth = i_depth + 1
          continue

      ## SAO = Germline
      if STR_ORIGIN in dict_info_tokens:
        if ( dict_info_tokens[ STR_ORIGIN ] == STR_ORIGIN_GERMLINE ) and ( STR_COSMIC_ID not in dict_info_tokens ):
          i_origin = i_origin + 1
          continue

      ## FATHMM = Neutral or passenger
      if STR_FATHMM in dict_info_tokens:
        if ( dict_info_tokens[ STR_FATHMM ] == STR_FATHMM_NEUTRAL ) and ( STR_COSMIC_ID not in dict_info_tokens ):
          i_fathmm = i_fathmm + 1
          continue
      ...
      # TODO could add
      # SSR,Number=1,Type=Integer,Description="Variant Suspect Reason Codes (may be more than one value added together) 0 - unspecified, 1 - Paralog, 2 - byEST, 4 - oldAlign, 8 - Para_EST, 16 - 1kg_failed, 1024 - other"
      # SSR
