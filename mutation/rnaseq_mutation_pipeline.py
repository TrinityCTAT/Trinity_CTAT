...
def func_do_star_alignment(args_call,
                           str_unique_id,
                           pline_cur,
                           f_index_only=False):
    """
    ...
    * str_unique_id : Unique key string for this sample run,
                    ...
                    : String
    * pline_cur : Instance of the pipeline
                : Pipeline
    ...
    * return : Dict containing commands, resulting alignemnt file and the
    ...
    str_star_output_bai = str_mapped_bam + ".bai"
    ...
    #str_star_output_bam = os.path.join(str_misc_dir, "Aligned.out.bam")
    ...
    #if not f_index_only:
    #    lstr_dirs_to_make.extend([str_index_dir_2,
    #                              str_align_dir_1,
    #                              str_align_dir_2])
    ...
    if not f_index_only:
        ...
        if str_ext_left != str_ext_right:
            print(" ".join([ "Fastq files from a single sample should both",
        ...
        # Star aligner can leave unmapped reads in the bam by default due to
        # a bug in certain versions. GATK does not like this. Until the bug-fix
        # is in a standard release we will remove the reads with samtools
        str_remove_unmapped = " ".join(["samtools view -hbF 12",
                                       str_star_sorted_bam,
                                       ">", str_mapped_bam])
        cmd_star_unmap = Command.Command(str_cur_command=str_remove_unmapped,
                                        lstr_cur_dependencies=[str_star_sorted_bam],
                                        lstr_cur_products=[str_mapped_bam])
        lcmd_commands.append(cmd_star_unmap)

        # Create bai
        str_star_bai = " ".join(["samtools index", str_mapped_bam])
        cmd_bai = Command.Command(str_cur_command=str_star_bai,
                                  lstr_cur_dependencies=[str_mapped_bam],
                                  lstr_cur_products=[str_star_output_bai])
        cmd_bai.func_set_dependency_clean_level([str_mapped_bam],
                                                 Command.CLEAN_NEVER)
        lcmd_commands.append(cmd_bai)

    return({INDEX_CMD:lcmd_commands,
            INDEX_FILE:str_mapped_bam,
            INDEX_FOLDER:str_misc_dir})


def func_do_gsnp_alignment(args_call, str_unique_id,
                           pline_cur, f_index_only=False):
    """
    Manages the commands for gsnp alignment.
    ...
    * f_index_only : Indicates that indexing only should result.
                   : Boolean (True indicates indexing only, no alignment)
    * return : Dict containing commands, resulting alignemnt
               file and the alignment folder
             : dict
    """
    ...
    str_gsnap_sorted_bam = os.path.join(str_align_dir, STR_BAM_SORTED_FILE_NAME)
    ...
    if not f_index_only:
        ...
        lcmd_commands.append(Command.Command(str_cur_command = " ".join(["samtools sort -O bam -T " + str_temp_prefix + " -o", str_gsnap_sorted_bam, str_output_file]),
                                               lstr_cur_dependencies = [str_output_file],
                                               lstr_cur_products = [str_gsnap_sorted_bam]))
        # Create bai
        lcmd_commands.append(Command.Command(str_cur_command = " ".join(["samtools index", str_gsnap_sorted_bam]),
                              lstr_cur_dependencies = [str_gsnap_sorted_bam],
                              lstr_cur_products = [str_bai_file]))


    return { INDEX_CMD : lcmd_commands, INDEX_FILE : str_gsnap_sorted_bam , INDEX_FOLDER : str_align_dir }
...
def func_do_BWA_alignment(args_call, str_unique_id, pline_cur, f_index_only = False):
    ...
    * return : Dict containing commands, resulting alignemnt file and the alignment folder
    ...
    str_bwa_sorted_bam = os.path.join(args_call.str_file_base, ".".join([str_left_file_key, "sorted", "bam"]))
    ...
    return { INDEX_CMD: lcmd_dna_mapping_commands, INDEX_FILE: str_bwa_sorted_bam, INDEX_FOLDER:args_call.str_file_base }
...
def func_do_rnaseq_caller_gatk(args_call,
                               str_input_bam,
                               str_unique_id,
                               str_project_dir,
                               str_tmp_dir):
...
#        str_depth_compressed_file = os.path.basename(args_call.str_file_base) + ".depth.gz"
        ...
        str_depth_compressed_file = os.path.join(args_call.str_file_base,
...
def func_do_variant_calling_gatk(args_call,
                                 str_align_file,
                                 str_unique_id,
                                 str_project_dir,
                                 str_tmp_dir,
    ...
    * return: List of commands
    ...
    dict_recal = func_do_recalibration_gatk(args_call,
                                            str_align_file,
                                            str_unique_id,
                                            str_project_dir,
                                            str_tmp_dir,
    ...
    # Do calling
    dict_rnaseq_gatk = func_do_rnaseq_caller_gatk(args_call,
...
def func_do_variant_calling_samtools(args_call, str_align_file, str_unique_id, str_project_dir, str_tmp_dir, lstr_dependencies, logr_cur):
    ...
    * logr_cur : Pipeline logger
    ...
    if os.path.splitext(str_align_file)[1].lower() == ".sam":
        ...
        str_bam_sorted = os.path.join(str_tmp_dir, func_replace_extension(str_bam_file, "_sorted.bam"))
    ...
    if args_call.f_recalibrate_sam:
        ...
        str_bam_sorted_index = func_replace_extension(str_bam_sorted, "bai")
...
def func_do_variant_calling_none(args_call, str_align_file,
...
def func_do_filtering_bcftools(args_call, str_variants_file,
    ...
    * logr_cur : Pipeline logger
    ...
    str_standard_variants_file = args_call.str_file_base + os.sep + C_STR_INIT_FILTER
    str_filtered_variants_file = func_replace_extension(str_variants_file, "_decluster.vcf")
...
def func_do_filtering_gatk(args_call, str_variants_file,
                           lstr_dependencies, logr_cur):
    """
    ...
    * logr_cur: Pipeline logger
    ...
    """
    # Filtered variants file
    str_filtered_variants_file = os.path.join(args_call.str_file_base,
                                              C_STR_INIT_FILTER)
    str_filtered_variants_index_file = str_filtered_variants_file + ".csi"
    # Filter variants
...
def func_do_filtering_none(args_call, str_variants_file,
                                   lstr_dependencies, logr_cur):
    """
    Creates the commands for the gatk hard filtering.
    * args_call: Arguments for the pipeline
...
def func_do_variant_filtering_cancer(args_call, str_variants_file,
    ...
    * return: List of commands
    ...
    str_cancer_mutations_filtered = func_replace_extension(str_vcf_base, "_cosmic_filtered.vcf")
    str_cravat_annotated_coding_vcf = func_replace_extension(str_vcf_base, "_cosmic_filtered_cravate_annotated_coding.vcf.gz")
    str_cravat_annotated_all_vcf = str_project_dir + os.path.sep + "annotated_min_filtered.vcf.gz"
    str_cravat_filtered_vcf = func_replace_extension(str_vcf_base, "_cosmic_filtered_cravate_annotated_filtered.vcf")
    ...
    str_cravat_result_dir = func_replace_extension(str_vcf_base, "_cosmic_filtered_cravat_annotations.gz")
    ...
    if not f_is_hg_18 is None:
        ...
        cmd_filter_with_cravat.func_set_dependency_clean_level([str_cravat_annotated_all_vcf], Command.CLEAN_NEVER)
...
def run(args_call, f_do_index = False):
    ...
    * f_do_index : Boolean
    ...
    if not (args_call.str_sample_file_left_fq and args_call.str_sample_file_right_fq) and not args_call.str_bam_file:
      if not f_do_index:
          print(" ".join(["RNASEQ MUTATION PIPELINE, please make",
    ...
    dict_calling_funcs = {STR_DNASEQ_VALIDATION:func_call_dnaseq_like_rnaseq,
    ...
    dict_filtering_funcs = {STR_FILTERING_BCFTOOLS:func_do_filtering_bcftools,
                            STR_FILTERING_GATK:func_do_filtering_gatk,
                            STR_FILTERING_NONE:func_do_filtering_none}
    ...
    if args_call.str_bam_file:
        dict_align_info = {INDEX_FILE:args_call.str_bam_file,
                           INDEX_FOLDER:args_call.str_bam_file}
    ...
    if args_call.f_calculate_base_coverage and (args_call.str_variant_call_mode == STR_VARIANT_NONE):
        # Create depth file
#        str_depth_compressed_file = os.path.basename(func_replace_extension(dict_align_info[INDEX_FILE], "_depth.gz"))
        str_depth_compressed_file = os.path.basename(func_replace_extension(dict_align_info[INDEX_FILE], "depth"))
        str_depth_compressed_file = os.path.join(args_call.str_file_base, str_depth_compressed_file)
#        lcmd_commands.append(Command.Command(str_cur_command = "samtools depth " + dict_align_info[INDEX_FILE] + " | gzip > " + str_depth_compressed_file,
    ...
    if(args_call.str_variant_call_mode.lower() != STR_VARIANT_NONE.lower()):
        ...
        func_calling = dict_calling_funcs[args_call.str_variant_call_mode]
        dict_ret_variant_calling = func_calling(args_call=args_call,
        ...
        func_filtering = dict_filtering_funcs[args_call.str_variant_filter_mode]
        dict_ret_variant_filtration = func_filtering(args_call=args_call,
        ...
        if not args.f_validate_by_dnaseq:
            ...
            str_clean_vcf = os.path.join(str_misc_dir,
                                         func_replace_extension(os.path.basename(str_annotated_vcf_file),
                                         "_clean.vcf"))
...
#                lcmd_commands.extend(func_plot_vcf(str_annotated_vcf_file)[INDEX_CMD])
            ...
            dict_sample_csi = func_csi(str_annotated_vcf_file,
                                       args_call.str_file_base)
            str_annotated_vcf_file = dict_sample_csi[INDEX_FILE]
            str_csi_vcf_file = os.path.splitext(str_annotated_vcf_file)[0] + ".csi"
            ...
            str_csi_dbsnp = args_call.str_vcf_file + ".csi"
...
def func_gz(str_vcf, str_output_dir = ""):
  """
...
def func_replace_extension(str_file_path, str_new_extension):
    """
    Adds a new extension in the place of an older extension.

    @params str_file_path: File path to update (should not contain .
                           unless followed by an extension)
                         : String
    @params str_new_extension: New extension to add
                             : String
    @return: Updated File path
           : String
    """

    if (not str_file_path) or (not str_new_extension):
    ...
    if not str_dir:
        ...
        return("".join([str_dir, os.sep,
                        str_file_name.split(".")[0], str_new_extension]))
